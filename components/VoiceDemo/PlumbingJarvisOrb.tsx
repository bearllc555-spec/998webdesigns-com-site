...
/** Plumbing voice orb — dark core, amplified rim driven by Jarvis audio levels. */
export function PlumbingJarvisOrb({
  ...
  const ringDrive = active ? Math.min(1, l.volume * 0.55 + l.mid * 0.3 + l.treble * 0.15) : 0;
  const ringScale = 1 + ringDrive * 0.14 + (speaking ? l.bass * 0.06 : 0);
  const outerPulse = 1.06 + ringDrive * 0.22;
  const rimWidth = 2.5 + ringDrive * 3.5;
  const rimGlow = 12 + ringDrive * 36;
  const rimInnerGlow = 6 + l.treble * 18;
  const rimOpacity = active ? 0.55 + ringDrive * 0.45 : 0.28;
  const outerRingOpacity = active ? 0.2 + ringDrive * 0.55 : 0.08;
  const cardHalo = active ? 0.12 + ringDrive * 0.4 : 0.06;

  return (
    <div className="relative flex h-28 w-28 items-center justify-center" aria-hidden>
      {/* Card halo — follows voice */}
      <div
      ...
      {/* Outer voice ring — mid/treble shimmer */}
      <div
      ...
      {/* Primary rim — amplified + voice-scaled */}
      <div
        ...
        style={{
          ...
          boxShadow: speaking
            ...
            : `0 0 ${rimGlow * 0.45}px rgba(56, 189, 248, ${0.25 + rimOpacity * 0.2})`,
        }}
      />
