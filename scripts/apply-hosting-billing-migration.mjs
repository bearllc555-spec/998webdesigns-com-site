/**
 * Apply lifetime hosting day-31 columns on helmet (xwldbxburzqryxlzocck).
 * Uses same credentials as apply-subscription-column.mjs.
 */
import fs from "fs";
...
const migrationPath = path.join(
  ...
  "supabase",
  "migrations",
  "20260605180000_wd_leads_hosting_billing.sql"
);
  ...
  for (const config of configs) {
    ...
    try {
      ...
      const cols = await client.query(
        `select column_name from information_schema.columns where table_schema = 'public' and table_name = 'wd_leads' and column_name in ('hosting_billing_starts_at','stripe_ten_year_session_id','ten_year_hosting_paid_at') order by 1`
      );
      console.log(
        cols.rows.length === 3
