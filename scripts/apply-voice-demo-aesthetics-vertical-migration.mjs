/**
 * Apply voice_demo_leads aesthetics vertical check on helmet (xwldbxburzqryxlzocck).
 */
import fs from "fs";
...
const migrationPath = path.join(
  ...
  "supabase",
  "migrations",
  "20260626120000_voice_demo_leads_aesthetics_vertical.sql"
);
...
async function main() {
  ...
  for (const config of configs) {
    ...
    try {
      ...
      const def = verify.rows[0]?.def ?? "(missing)";
      console.log(`OK - voice_demo_leads_vertical_check: ${def}`);
      await client.end();
      return;
    } catch (err) {
      lastErr = err;
