/**
 * Apply jarvis_plumbing_jobs table on helmet (xwldbxburzqryxlzocck).
 */
import fs from "fs";
...
const migrationPath = path.join(
  ...
  "supabase",
  "migrations",
  "20260608120000_jarvis_plumbing_jobs.sql"
);
    ...
    try {
      ...
      console.log(
        table.rows.length >= 10
          ? `OK — jarvis_plumbing_jobs exists (${table.rows.length} columns)`
          : `WARN — columns found: ${table.rows.map((r) => r.column_name).join(", ")}`
      );
      await client.end();
      return;
    } catch (err) {
