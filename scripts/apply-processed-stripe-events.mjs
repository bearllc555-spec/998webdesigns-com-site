/**
 * Apply processed_stripe_events migration using POSTGRES_* from .env.vercel.prod.
 * Usage: node scripts/apply-processed-stripe-events.mjs [.env.vercel.prod]
 */
import fs from "fs";
...
const migrationPath = path.join(
  ...
  "supabase",
  "migrations",
  "20260605140000_processed_stripe_events.sql"
);
  ...
  for (const config of configs) {
    ...
    try {
      ...
      const check = await client.query(
        `select table_name from information_schema.tables where table_schema = 'public' and table_name = 'processed_stripe_events'`
      );
