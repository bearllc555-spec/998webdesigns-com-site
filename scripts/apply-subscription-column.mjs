/**
 * Apply stripe_subscription_id column migration on helmet (xwldbxburzqryxlzocck).
 * Uses same credentials as apply-helmet-schema.mjs.
 */
import fs from "fs";
...
const migrationPath = path.join(
  ...
  "supabase",
  "migrations",
  "20260602120000_wd_leads_stripe_subscription.sql"
);
  ...
  for (const config of configs) {
    ...
    try {
      ...
      const col = await client.query(
        `select column_name from information_schema.columns where table_schema = 'public' and table_name = 'wd_leads' and column_name = 'stripe_subscription_id'`
      );
      console.log(
        col.rows.length
