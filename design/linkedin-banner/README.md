...
**Upload export (1584×396):** click the cover on any preview page (opens PNG in a new tab — right-click → Save image as), or open `/temp/export` … `/temp/4/export` directly. Each export tab also has a **Download PNG** link.

New iterations → `/temp/5`, … Add a row in `lib/linkedin-banner-preview.ts` (`BANNER_DESIGN_ROUTES`).
