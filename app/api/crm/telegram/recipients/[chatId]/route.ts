...
import {
  removeTelegramRecipientFromCrm,
  updateTelegramRecipientFromCrm,
...
export async function PATCH(
  ...
  const oldId = decodeURIComponent(chatId).trim();
  if (!oldId) {
    return NextResponse.json({ error: "Invalid chat id" }, { status: 400 });
  }

  let body: { label?: string; chatId?: string };
  try {
  ...
  if (body.label === undefined && body.chatId === undefined) {
    return NextResponse.json({ error: "Nothing to update" }, { status: 400 });
  }

  const result = await updateTelegramRecipientFromCrm(oldId, body);
  if (!result.ok) {
