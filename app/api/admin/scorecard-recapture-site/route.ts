...
import { recaptureScorecardSiteByToken } from "@/lib/scorecard/recapture-site";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/** POST - re-shoot site thumbnail via VPS generator. Bearer: BALANCE_CAPTURE_SECRET. Body: { token } */
export async function POST(req: NextRequest) {
  ...
  const result = await recaptureScorecardSiteByToken(token);
  if (!result.ok) {
