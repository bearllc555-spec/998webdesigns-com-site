...
import { runScorecardMigration } from "@/lib/pg-migrate-scorecard";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/** POST - scorecard tables + RLS RPCs (idempotent). Bearer: BALANCE_CAPTURE_SECRET. */
export async function POST(req: NextRequest) {
  ...
  const result = await runScorecardMigration();
  if (!result.ok) {
