...
import { runVoiceDemoAestheticsVerticalMigration } from "@/lib/pg-migrate-voice-demo-aesthetics-vertical";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/** POST - extend voice_demo_leads.vertical check for clinical/wellness (idempotent). Bearer: BALANCE_CAPTURE_SECRET. */
export async function POST(req: NextRequest) {
  ...
  const result = await runVoiceDemoAestheticsVerticalMigration();
  if (!result.ok) {
