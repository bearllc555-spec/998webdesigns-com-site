...
import { runRlsSecurityMigration } from "@/lib/pg-migrate-rls-security";
...
export async function POST(req: NextRequest) {
  ...
  const result = await runRlsSecurityMigration();
  if (!result.ok) {
    return NextResponse.json({ error: result.detail }, { status: 500 });
  }

  return NextResponse.json({ ok: true, via: result.via, tables: result.tables });
}
