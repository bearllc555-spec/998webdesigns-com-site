...
import { runVoiceDemoLocationMigration } from "@/lib/pg-migrate-voice-demo-location";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/** POST — add voice_demo_leads location columns (idempotent). Bearer: BALANCE_CAPTURE_SECRET. */
export async function POST(req: NextRequest) {
  ...
  const result = await runVoiceDemoLocationMigration();
  if (!result.ok) {
