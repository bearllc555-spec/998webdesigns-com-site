...
import { runLinkedinProspectsMigration } from "@/lib/pg-migrate-linkedin-prospects";
import { enforceAdminRateLimit, rateLimitResponse } from "@/lib/api-rate-limit";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/** POST - create linkedin_prospects table (idempotent). Bearer: BALANCE_CAPTURE_SECRET. */
export async function POST(req: NextRequest) {
  ...
  const result = await runLinkedinProspectsMigration();
  if (!result.ok) {
