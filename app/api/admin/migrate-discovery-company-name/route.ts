...
import { runDiscoveryCompanyNameMigration } from "@/lib/pg-migrate-discovery-company-name";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/** POST - add discovery_prospects.company_name (idempotent). Bearer: BALANCE_CAPTURE_SECRET. */
export async function POST(req: NextRequest) {
  ...
  const result = await runDiscoveryCompanyNameMigration();
  if (!result.ok) {
