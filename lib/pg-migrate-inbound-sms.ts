...
const MIGRATION_SQL = fs.readFileSync(
  path.join(process.cwd(), "supabase/migrations/20260606230000_inbound_sms.sql"),
  "utf8"
);
...
export async function runInboundSmsMigration(): Promise<
  ...
  for (const config of configs) {
    ...
    try {
      ...
      const check = await client.query(
        `select table_name from information_schema.tables where table_schema = 'public' and table_name = 'inbound_sms'`
      );
      await client.end();
      if (!check.rowCount) {
        return { ok: false, detail: `Migration ran on ${via} but inbound_sms not found` };
      }
      return { ok: true, via };
    } catch (err) {
