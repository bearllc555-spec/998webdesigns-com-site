...
const MIGRATION_SQL = fs.readFileSync(
  path.join(process.cwd(), "supabase/migrations/20260606120000_crm_inbox_flag.sql"),
  "utf8"
);
...
export async function runCrmInboxFlagMigration(): Promise<
    ...
    try {
      ...
      const check = await client.query(
        `select 1 from information_schema.columns
         where table_schema = 'public' and table_name = 'wd_leads' and column_name = 'inbox_flag'`
      );
      await client.end();
      if (!check.rowCount) {
        return { ok: false, detail: `Migration ran on ${via} but inbox_flag column not found` };
      }
      return { ok: true, via };
    } catch (err) {
