...
/** Send scorecard_ready Telegram once per report (atomic claim on crm_ready_notified_at). */
export async function notifyScorecardReadyOnce(
  ...
  if (error) {
    if (/crm_ready_notified_at|does not exist/i.test(error.message)) {
      ...
      console.warn("[scorecard-ready-notify] claim failed:", error.message);
    }
    return false;
  }
  ...
  try {
    ...
    console.warn("[scorecard-ready-notify] telegram failed, clearing claim:", err);
    await supa
      .from("scorecard_reports")
      .update({ crm_ready_notified_at: null })
      .eq("id", input.reportId);
    return false;
  }
}

/** Reports emailed but never Telegram-notified (VPS env gap, closed browser, etc.). */
export async function backfillScorecardReadyTelegram(limit = 15): Promise<number> {
  ...
  const { data: rows, error } = await supa
    .from("scorecard_reports")
    .select(
      "id, token, domain, business_name, score, verdict, lead_id, leads ( full_name, email, phone )"
    )
    .eq("status", "active")
