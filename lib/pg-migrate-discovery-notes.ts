...
const MIGRATION_SQL = fs.readFileSync(
  path.join(process.cwd(), "supabase/migrations/20260606220000_discovery_crm_notes.sql"),
  "utf8"
);
...
export async function runDiscoveryCrmNotesMigration(): Promise<
    ...
    try {
      ...
      const check = await client.query(
        `select column_name from information_schema.columns
         where table_schema = 'public' and table_name = 'discovery_prospects' and column_name = 'crm_notes'`
      );
      await client.end();
      if (!check.rowCount) {
        return {
          ok: false,
          detail: `Migration ran on ${via} but discovery_prospects.crm_notes not found`,
        };
      }
      return { ok: true, via };
    } catch (err) {
