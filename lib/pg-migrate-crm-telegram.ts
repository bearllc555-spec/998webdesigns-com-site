...
const MIGRATION_SQL = fs.readFileSync(
  path.join(process.cwd(), "supabase/migrations/20260602180000_crm_telegram_settings.sql"),
  "utf8"
);
...
export async function runCrmTelegramMigration(): Promise<
  ...
  for (const config of configs) {
    ...
    try {
      ...
      const check = await client.query(
        `select 1 from information_schema.tables where table_schema = 'public' and table_name = 'crm_telegram_settings'`
      );
      await client.end();
      if (!check.rowCount) {
        return { ok: false, detail: `Migration ran on ${via} but table not found` };
      }
      return { ok: true, via };
    } catch (err) {
