...
const MIGRATION_SQL = fs.readFileSync(
  path.join(process.cwd(), "supabase/migrations/20260605180000_wd_leads_hosting_billing.sql"),
  "utf8"
);
...
export async function runHostingBillingMigration(): Promise<
  ...
  for (const config of configs) {
    ...
    try {
      ...
      const check = await client.query(
        `select column_name from information_schema.columns where table_schema = 'public' and table_name = 'wd_leads' and column_name = 'hosting_billing_starts_at'`
      );
      await client.end();
      if (!check.rowCount) {
        return { ok: false, detail: `Migration ran on ${via} but hosting_billing_starts_at not found` };
      }
      return { ok: true, via };
    } catch (err) {
