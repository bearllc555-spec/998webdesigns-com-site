...
import {
  voiceDemoConfirmEmailSystemPrompt,
...
export async function createVoiceDemoLiveTokenForEmailConfirm(
  email: string
  ...
  const systemInstruction = voiceDemoConfirmEmailSystemPrompt(email);
  const mode: VoiceDemoToolMode = "confirm_email";
    ...
    const detail =
      err instanceof Error
        ...
        : typeof err === "object" && err !== null
          ? JSON.stringify(err)
          : String(err);
    console.warn("[voice-demo-live-token] confirm_email", detail);
