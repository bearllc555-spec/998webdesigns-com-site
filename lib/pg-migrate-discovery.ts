...
const MIGRATION_SQL = fs.readFileSync(
  path.join(process.cwd(), "supabase/migrations/20260606140000_discovery_prospects.sql"),
  "utf8"
);
...
export async function runDiscoveryMigration(): Promise<
  ...
  for (const config of configs) {
    ...
    try {
      ...
      const check = await client.query(
        `select table_name from information_schema.tables where table_schema = 'public' and table_name = 'discovery_prospects'`
      );
      await client.end();
      if (!check.rowCount) {
        return { ok: false, detail: `Migration ran on ${via} but discovery_prospects not found` };
      }
      return { ok: true, via };
    } catch (err) {
