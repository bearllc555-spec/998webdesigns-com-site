...
describe("design payment schedule", () => {
  it("splits 50 / 40 / 10 of list design fee", () => {
    expect(designTotalCents()).toBe(599800);
    expect(designDepositCents()).toBe(299900);
    expect(designMilestone2Cents()).toBe(239920);
    expect(designMilestone3Cents()).toBe(59980);
    expect(
  ...
  it("checkout subtotal is deposit or full", () => {
    expect(checkoutDesignSubtotalCents("deposit")).toBe(299900);
    expect(checkoutDesignSubtotalCents("full")).toBe(599800);
  });

  it("balance after deposit equals milestones 2 + 3", () => {
    expect(designBalanceAfterDepositCents()).toBe(299900);
